      ...
      {isReady ? (
        <>
          {/* `object-contain` keeps the full image visible if the
              ...
              `thumbnailUrl` is the ~800px JPEG generated by the webhook
              alongside the full PNG — ~95% smaller, perfect for the
              grid. Falls back to `imageUrl` for legacy rows (pre-thumb
              column) or post-edit rows where the stale thumbnail was
              cleared. `loading="lazy"` defers off-screen tiles so a
              project page with many variants doesn't bank-fetch them
              all at once. */}
          <img
